...
status=$(ssh $MASTER /opt/membase/bin/cli/membase server-list -c $MASTER -u Administrator -p password -o json | sed -e 's/^.*"balanced":\(.*\),"rebalanceStatus.*$/\1/')
