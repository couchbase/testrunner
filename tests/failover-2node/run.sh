...
ssh $MASTER /opt/membase/bin/cli/membase server-add -c localhost -u Administrator -p password --server-add=$SLAVE --server-add-username=Administrator --server-add-password=password &> /dev/null
ssh $MASTER /opt/membase/bin/cli/membase rebalance -c localhost -u Administrator -p password &> /dev/null
...
ssh $MASTER /opt/membase/bin/cli/membase failover -c localhost -u Administrator -p password --server-failover $SLAVE &> /dev/null
sleep 5
count=$(ssh $MASTER /opt/membase/bin/cli/membase server-list -c $MASTER -u Administrator -p password 2> /dev/null | grep "healthy active" | wc -l)
if [[ $count -ne 1 ]] ; then
