...
class UpgradePartialParititionedIndex(UpgradeSecondaryIndex):
    ...
    def test_full_upgrade(self):
        ...
        self.assertEquals(pending_results['results'], initial_expected)
        self.assertEquals(success_results['metrics']['resultCount'], 0)
        self.assertEquals(error_results['metrics']['resultCount'], 0)
        ...
        if self.partial_partitioned:
            self.assertEquals(pending_results['results'], initial_expected)
            self.assertEquals(success_results['results'], success_expected)
            self.assertEquals(error_results['results'], error_expected)
        else:
            self.assertEquals(pending_results['metrics']['resultCount'], 0)
            self.assertEquals(success_results['results'], success_expected)
            self.assertEquals(error_results['results'], error_expected)
            ...
            if key in update_pending:
                ...
                if key == "K1":
                    if self.mixed_mode_kv:
                        ...
                        success_expected = [{u'id': u'K1'},{u'id': u'K2'}, {u'id': u'K3'}]
                        self.assertEquals(pending_results['results'], pending_expected)
                        self.assertEquals(success_results['results'], success_expected)
                    else:
        ...
        if self.mixed_mode_kv:
            self.assertEquals(error_results['metrics']['resultCount'], 1)
        else:
            self.assertEquals(error_results['metrics']['resultCount'], 0)
