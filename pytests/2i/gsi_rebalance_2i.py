...
##############################################################################################
    ...
    def test_gsi_rebalance_in_query_node(self):
        self.run_operation(phase="before")
        self.sleep(30)
        self.n1ql_helper.run_cbq_query(query="PREPARE p1 from select * from default limit 5", server=self.servers[0])
        self.sleep(5)
        for i in range(self.nodes_init):
            self.n1ql_helper.run_cbq_query(query="execute p1", server=self.servers[i])
        map_before_rebalance, stats_map_before_rebalance = self._return_maps()
        # rebalance in a node
    ...
    def test_query_rebalance_swap_rebalance(self):
        self.run_operation(phase="before")
        self.sleep(30)
        self.n1ql_helper.run_cbq_query(query="PREPARE p1 from select * from default limit 5", server=self.servers[0])
        self.sleep(5)
