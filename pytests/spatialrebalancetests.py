...
class SpatialRebalanceTests(unittest.TestCase):
    ...
    def test_delete_10k_docs_create_cluster(self):
        self._test_delete_docs_create_cluster(100000, 10000)

    def test_delete_10k_docs_destroy_cluster(self):
        self._test_delete_docs_destroy_cluster(100000, 10000)

    def test_insert_10k_docs_during_rebalance(self):
        self._test_insert_docs_during_rebalance(10000)


    def _test_delete_docs_create_cluster(self, num_of_docs,
                                        num_of_deleted_docs):
        msg = "description : have a single node, insert {0} docs, "\
            "delete {1} docs while creating a cluster and query it"
        self.log.info(msg.format(num_of_docs, num_of_deleted_docs))
        ...
        # spatial view
        results = self.helper.get_results(design_name, num_of_docs)
    ...
    def _test_delete_docs_destroy_cluster(self, num_of_docs,
                                          num_of_deleted_docs):
    ...
    def _test_insert_docs_during_rebalance(self, num_of_docs):
        ...
        for server in self.helper.servers[1:]:
            ...
            inserted_keys.extend(self.helper.insert_docs(
                    num_of_docs, prefix, wait_for_persistence=False))
            self._wait_for_rebalance()
