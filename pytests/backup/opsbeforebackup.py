...
class OpsBeforeBackupTests(BackupBaseTest):
    ...
    def CreateUpdateDeleteBeforeBackup(self):
        ...
        self._load_all_buckets(self.master, gen_load_mysql, "create", 0, 1, self.item_flag, True)
        self._load_all_buckets(self.master, gen_load_couchdb, "create", 0, 1, self.item_flag, True)

        if(self.doc_ops is not None):
            if("update" in self.doc_ops):
                self._load_all_buckets(self.master, gen_update, "update", 0, 1, self.item_flag, True)
            if("create" in self.doc_ops):
                self._load_all_buckets(self.master, gen_create, "create", 0, 1, self.item_flag, True)
            if("delete" in self.doc_ops):
                self._load_all_buckets(self.master, gen_delete, "delete", 0, 1, self.item_flag, True)
    ...
    def CreateUpdateDeleteExpireBeforeBackup(self):
        ...
        self._load_all_buckets(self.master, gen_load, "create", 0, 1, self.item_flag, True)
        extra_items_deleted_flag = 0

        if(self.doc_ops is not None):
            self._load_all_buckets(self.master, gen_extra, "create", 0, 1, self.item_flag, True)
            if("update" in self.doc_ops):
                self._load_all_buckets(self.master, gen_extra, "update", 0, 1, self.item_flag, True)
            if("delete" in self.doc_ops):
                self._load_all_buckets(self.master, gen_extra, "delete", 0, 1, self.item_flag, True)
