...
class CouchbaseCliTestWithMeta(CliBaseTest):
    ...
    def test_epengine_save_meta_to_48bits_in_ram(self):
        """
            ...
            This fix went into version 4.6.5 and later.
            params: eviction_policy=fullEviction,sasl_buckets=1
        """
        if self.cb_version[:5] < "4.6.5" :
            self.log.info("This test only work for version 4.6.5 and later.")
            return
        ...
        mc = MemcachedClient(self.master.ip, 11211)
        mc.sasl_auth_plain('bucket0', 'password')
        mc.vbucketId = 903
