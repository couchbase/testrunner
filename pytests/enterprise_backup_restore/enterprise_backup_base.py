...
class EnterpriseBackupRestoreBase(BaseTestCase):
    def setUp(self):
        ...
        exclude_buckets = self.input.param("exclude-buckets", None)
        include_buckets = self.input.param("include-buckets", None)
        ...
        filter_keys = self.input.param("filter-keys", None)
        filter_values = self.input.param("filter-values", None)
        ...
        self.backupset = BackupSet(backup_host, backup_cluster_host, cluster_host_username,
                                   cluster_host_password, directory, backup_name, exclude_buckets,
    ...
    def backup_restore(self):
        ...
        args = "restore --dir {0} --name {1} --host http://{2}:{3} --username {4} --password {5} --start {6} --end {7}". \
            format(self.backupset.dir, self.backupset.name, self.backupset.cluster_host.ip,
                   self.backupset.cluster_host.port, self.backupset.cluster_host_username,
                   self.backupset.cluster_host_password, backup_start, backup_end)
        if self.backupset.exclude_buckets:
            args += "--exclude-buckets {0}".format(self.backupset.exclude_buckets)
        if self.backupset.include_buckets:
            args += "--include-buckets {0}".format(self.backupset.include_buckets)
...
class BackupSet(object):
    def __init__(self, backup_host, cluster_host, cluster_host_username,
                 cluster_host_password, directory="/tmp/entbackup", name="backup",
        ...
        self.exclude_buckets = exclude_buckets
