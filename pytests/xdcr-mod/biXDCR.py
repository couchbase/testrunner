...
Verifying whether XDCR replication is successful on subsequent destination clusters. """
    ...
    def load_with_async_ops_with_warmup(self):
        ...
        #warmup
        all_nodes = src_nodes
        all_nodes.extend(dest_nodes)
        #for warmupnode in all_nodes:
    ...
    def load_with_async_ops_and_joint_sets_with_warmup(self):
        ...
        all_nodes.extend(dest_nodes)
        for warmupnode in all_nodes:
            shell = RemoteMachineShellConnection(warmupnode)
            shell.stop_couchbase()
            print "Just stopped ", warmupnode.ip
            time.sleep(10)
            shell.start_couchbase()
            print "And started it back up again!"
            shell.disconnect()
        time.sleep(60)
