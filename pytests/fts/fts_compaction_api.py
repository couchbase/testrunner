...
class FTSIndexCompactionAPI(FTSBaseTest):
    ...
    def test_start_compaction(self):
        ...
        for idx in self._cb_cluster.get_indexes():
            ...
            self.assertTrue(start_compaction_status, "Index compaction did not started, response is: \n{0}".format(start_compaction_content))
    ...
    def test_start_compaction_sequential(self):
        ...
        for idx in self._cb_cluster.get_indexes():
            ...
            self.assertTrue(start_compaction_status_again, "Second sequential index compaction call is failed, "
                                                           "response is: \n{0}".format(start_compaction_content))
    ...
    def test_start_alias_compaction(self):
        ...
        self.assertFalse(start_compaction_status,
                         "Index alias compaction started unexpectedly, response is: \n{0}".format(start_compaction_content))
    ...
    def test_start_compaction_and_mutations_concurrently(self):
        ...
        for bucket in self._cb_cluster.get_buckets():
            self._cb_cluster.run_n1ql_query('create primary index on {0}'.format(bucket.name))
    ...
    def test_start_compaction_missed_index_negative(self):
        ...
        self.assertFalse(start_compaction_status, "Index compaction for missed index is started, response is: \n{0}".format(start_compaction_content))
        self.assertTrue("index not found" in start_compaction_content['error'], "Unexpected error message, response is: \n{}".format(start_compaction_content))
    ...
    def test_cancel_compaction(self):
        ...
        for idx in self._cb_cluster.get_indexes():
            ...
            self.assertFalse(cancel_key in compaction_keys_after_cancel, "Compaction cancellation is failed. \n"
                                                                         "Response is: {0}".format(cancel_response))
    ...
    def _call_compaction(self, index_name=None):
        ...
        self.assertTrue(status, "FTS index compaction REST call is failed. Response is: \n {0}".format(content))
        return status, content

    def _call_mutations(self, bucket_name='default'):
        self._cb_cluster.run_n1ql_query("update {0} set email=email||'.xxx'".format(bucket_name))
