...
class QueryNewTuqTests(QueryTests):
    ...
    def test_simple_check(self):
        ...
        for query_bucket in self.query_buckets:
            ...
            query = 'select * from {0}'.format(query_bucket)
            self.run_cbq_query(query)
            logging.debug("event is set")
            if self.monitoring:
                e.set()
                t1.join(100)
            query_template = 'FROM {0} select $str0, $str1 ORDER BY $str0,$str1 ASC'.format(query_bucket)
    ...
    def test_joins_monitoring(self):
        ...
        for query_bucket in self.query_buckets:
            ...
            if self.monitoring:
                ...
                t2.start()
            query = 'select * from {0} b1 inner join {0} b2 on keys b1.CurrencyCode inner join {0} b3 on keys' \
                    ' b1.CurrencyCode left outer join {0} b4 on keys b1.CurrencyCode'.format(query_bucket)
    ...
    def test_simple_negative_check(self):
        queries_errors = {'SELECT $str0 FROM {0} d WHERE COUNT(d.$str0)>3': 'Aggregates not allowed in WHERE',
    ...
    def test_unnest(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            query_template = 'SELECT emp.$int0, task FROM %s emp UNNEST emp.$nested_list_3l0 task' % query_bucket
            actual_result, expected_result = self.run_query_from_template(query_template)
            self._verify_results(actual_result['results'], expected_result)
    ...
    def test_subquery_select(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            self.query = 'SELECT $str0, $subquery(SELECT COUNT($str0) cn FROM %s d USE KEYS $5) as names ' \
                         'FROM %s' % (query_bucket, query_bucket)
    ...
    def test_subquery_from(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            self.query = 'SELECT tasks.$str0 FROM $subquery(SELECT $str0, $int0 FROM %s) as tasks' % query_bucket
    ...
    def test_consistent_simple_check(self):
        ...
        for query_bucket in self.query_buckets:
            actual_result1 = self.run_cbq_query(queries[0] % query_bucket)
            actual_result2 = self.run_cbq_query(queries[1] % query_bucket)
    ...
    def test_limit_offset(self):
        ...
        for query_bucket in self.query_buckets:
            if self.monitoring:
                ...
                t4.start()
            query_template = 'SELECT DISTINCT $str0 FROM %s ORDER BY $str0 LIMIT 10' % query_bucket
            actual_result, expected_result = self.run_query_from_template(query_template)
            if self.monitoring:
                e.set()
                t4.join(100)
            self._verify_results(actual_result['results'], expected_result)
            query_template = 'SELECT DISTINCT $str0 FROM %s ORDER BY $str0 LIMIT 10 OFFSET 10' % query_bucket
            actual_result, expected_result = self.run_query_from_template(query_template)

    def test_limit_offset_zero(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            query_template = 'SELECT DISTINCT $str0 FROM %s ORDER BY $str0 LIMIT 0' % query_bucket
            ...
            query_template = 'SELECT DISTINCT $str0 FROM %s ORDER BY $str0 LIMIT 10 OFFSET 0' % query_bucket
            actual_result, expected_result = self.run_query_from_template(query_template)
    ...
    def test_limit_offset_negative_check(self):
        queries_errors = {'SELECT DISTINCT $str0 FROM {0} LIMIT 1.1': 'Invalid LIMIT value 1.1',
                          'SELECT DISTINCT $str0 FROM {0} OFFSET 1.1': 'Invalid OFFSET value 1.1'}
        self.negative_common_body(queries_errors)

    def test_limit_offset_sp_char_check(self):
        queries_errors = {'SELECT DISTINCT $str0 FROM {0} LIMIT ~': 'syntax erro',
    ...
    def test_simple_alias(self):
        ...
        for query_bucket in self.query_buckets:
            if self.monitoring:
                ...
                t5.start()
            query_template = 'SELECT COUNT($str0) AS COUNT_EMPLOYEE FROM %s' % query_bucket
            if self.analytics:
                query_template = 'SELECT COUNT(`$str0`) AS COUNT_EMPLOYEE FROM %s' % query_bucket
            actual_result, expected_result = self.run_query_from_template(query_template)
            self.assertEqual(actual_result['results'], expected_result,
            ...
            query_template = 'SELECT COUNT(*) + 1 AS COUNT_EMPLOYEE FROM %s' % query_bucket
            actual_result, expected_result = self.run_query_from_template(query_template)
            if self.monitoring:
                e.set()
                t5.join(100)
    ...
    def test_alias_from_clause(self):
        ...
        for query_bucket in self.query_buckets:
            ...
            for query_template in queries_templates:
                actual_result, expected_result = self.run_query_from_template(query_template % query_bucket)
                if self.monitoring:
                    e.set()
                    t6.join(100)
                self._verify_results(actual_result['results'], expected_result)
    ...
    def test_alias_from_clause_group(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            query_template = 'SELECT $obj0.$_obj0_int0 AS points FROM %s AS test ' % query_bucket + \
    ...
    def test_alias_aggr_fn(self):
        ...
        for query_bucket in self.query_buckets:
            if self.monitoring:
                ...
                t8.start()
            query_template = 'SELECT COUNT(TEST.$str0) from %s AS TEST' % query_bucket
            if self.analytics:
                query_template = 'SELECT COUNT(TEST.`$str0`) from %s AS TEST' % query_bucket
    ...
    def test_order_by_check(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            query_template = 'SELECT $str0, $str1, $obj0.$_obj0_int0 points FROM %s' % query_bucket + \
                             ' ORDER BY $str1, $str0, $obj0.$_obj0_int0'
            actual_result, expected_result = self.run_query_from_template(query_template)
            self._verify_results(actual_result['results'], expected_result)
            query_template = 'SELECT $str0, $str1 FROM %s' % query_bucket + \
    ...
    def test_order_by_alias(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            query_template = 'SELECT $str1, $obj0 AS points FROM %s' % query_bucket + \
    ...
    def test_order_by_alias_arrays(self):
        ...
        for query_bucket in self.query_buckets:
            query_template = 'SELECT $str1, $obj0, $list_str0[0] AS SKILL FROM %s' % (
                query_bucket) + ' AS TEST ORDER BY SKILL, $str1, TEST.$obj0'
            actual_result, expected_result = self.run_query_from_template(query_template)
            self._verify_results(actual_result['results'], expected_result)
    ...
    def test_order_by_alias_aggr_fn(self):
        ...
        for query_bucket in self.query_buckets:
            query_template = 'SELECT $int0, $int1, count(*) AS emp_per_month from %s' % (
                query_bucket) + ' WHERE $int1 >7 GROUP BY $int0, $int1 ORDER BY emp_per_month, $int1, $int0'
            actual_result, expected_result = self.run_query_from_template(query_template)
    ...
    def test_order_by_aggr_fn(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            query_template = 'SELECT $str1 AS TITLE, min($int1) day FROM %s GROUP' % query_bucket + \
                             ' BY $str1 ORDER BY MIN($int1), $str1'
            actual_result, expected_result = self.run_query_from_template(query_template)
            self._verify_results(actual_result['results'], expected_result)

            if self.analytics:
                self.query = 'SELECT d.email AS TITLE, min(d.join_day) day FROM %s d GROUP' % query_bucket + \
    ...
    def test_order_by_satisfy(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            query_template = 'SELECT $str0, $list_obj0 FROM %s AS employee ' % query_bucket + \
    ...
    def test_distinct(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            query_template = 'SELECT DISTINCT $str1 FROM %s ORDER BY $str1' % query_bucket
            actual_result, expected_result = self.run_query_from_template(query_template)
            self._verify_results(actual_result['results'], expected_result)
    ...
    def test_distinct_nested(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            query_template = 'SELECT DISTINCT $obj0.$_obj0_int0 as VAR FROM %s ' % query_bucket + \
                             'ORDER BY $obj0.$_obj0_int0'
            actual_result, expected_result = self.run_query_from_template(query_template)
            self._verify_results(actual_result['results'], expected_result)

            query_template = 'SELECT DISTINCT $list_str0[0] as skill' + \
                             ' FROM %s ORDER BY $list_str0[0]' % query_bucket
            actual_result, expected_result = self.run_query_from_template(query_template)
            self._verify_results(actual_result['results'], expected_result)

            self.query = 'SELECT DISTINCT $obj0.* FROM %s' % query_bucket
            actual_result, expected_result = self.run_query_from_template(query_template)
            self._verify_results(actual_result['results'], expected_result)
    ...
    def test_simple_complex_paths(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            query_template = 'SELECT $_obj0_int0 FROM %s.$obj0' % query_bucket
            actual_result, expected_result = self.run_query_from_template(query_template)
            self._verify_results(actual_result['results'], expected_result)
    ...
    def test_alias_complex_paths(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            query_template = 'SELECT $_obj0_int0 as new_attribute FROM %s.$obj0' % query_bucket
            actual_result, expected_result = self.run_query_from_template(query_template)
            self._verify_results(actual_result['results'], expected_result)
    ...
    def test_where_complex_paths(self):
        self.fail_if_no_buckets()
        for query_bucket in self.query_buckets:
            query_template = 'SELECT $_obj0_int0 FROM %s.$obj0 WHERE $_obj0_int0 = 1' % query_bucket
            actual_result, expected_result = self.run_query_from_template(query_template)
            self._verify_results(actual_result['results'], expected_result)
