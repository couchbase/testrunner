...
class ScalarFunctionsTests(QueryTests):
    ...
    def setUp(self):
        super(ScalarFunctionsTests, self).setUp()
        self.query_bucket = self.get_query_buckets()[0]

    def suite_tearDown(self):
        super(ScalarFunctionsTests, self).suite_tearDown()

    def tearDown(self):
        super(ScalarFunctionsTests, self).tearDown()

    def test_nvl2_leading_non_null(self):
        test_dict = dict()
        count = 0

        param2 = [self.scalars_test_data['null'], self.scalars_test_data['missing'],
                  self.scalars_test_data['not_null_1']]
        param3 = [self.scalars_test_data['null'], self.scalars_test_data['missing'],
                  self.scalars_test_data['not_null_2']]
        ...
        for i in range(len(param2)):
            for j in range(len(param3)):
                ...
                query = ("select nvl2(" + self.scalars_test_data['not_null_1']['key'] +
                         ",%s,%s) from " + self.query_bucket + " USE KEYS ['plain'] where key1='" +
                ...
                lambda2 = lambda x, y=self.null_to_none(param2[i]['value']): self.assertEqual(
    ...
    def test_nvl_all_nulls(self):
        query = "select nvl(" + self.scalars_test_data['null'][
            'key'] + ", null) from " + self.query_bucket + " USE KEYS ['plain'] where key1='" + \
    ...
    def test_nvl_all_non_nulls(self):
        query = "select nvl(" + self.scalars_test_data['not_null_1']['key'] + ", " + \
                self.scalars_test_data['not_null_2']['key'] + \
                ") from " + self.query_bucket + " USE KEYS ['plain'] where key1='" + \
                self.scalars_test_data['not_null_1']['value'] + "'"
    ...
    def test_nvl_leading_null(self):
        query = "select nvl(" + self.scalars_test_data['null']['key'] + ", " + self.scalars_test_data['not_null_1'][
            'key'] + \
                ") from " + self.query_bucket + " USE KEYS ['plain'] where key1='" + \
                self.scalars_test_data['not_null_1']['value'] + "'"
    ...
    def test_nvl_trailing_null(self):
        query = "select nvl(" + self.scalars_test_data['not_null_1']['key'] + ", " + self.scalars_test_data['null'][
            'key'] + \
                ") from " + self.query_bucket + " USE KEYS ['plain'] where key1='" + \
                self.scalars_test_data['not_null_1']['value'] + "'"
            ...
            test_dict["%d-default" % count] = {"indexes": [],
                                               ...
                                               "asserts": [
                                                   ...
                                                   lambda x: self.assertTrue(
                                                       self.normalize_result(x['q_res'][0]) == expected_result)],
                                               "cleanups": []}
            count += 1
        self.query_runner(test_dict)
    ...
    def test_coalesce_correct_params(self):
        params = [self.scalars_test_data['not_null_1'], self.scalars_test_data['null'],
                  self.scalars_test_data['missing']]
        test_dict = dict()
        count = 0
        # 2 params cases
        for i in range(len(params)):
            for j in range(len(params)):
                ...
                query = ("select coalesce(%s,%s) from " + self.query_bucket + " USE KEYS ['plain'] where key1='" +
                ...
                lambda1 = lambda x: self.assertTrue(x['q_res'][0]['status'] == 'success')
                lambda2 = lambda x, y=self.null_to_none(params[i]['value']): self.assertEqual(
                    self.normalize_result(x['q_res'][0]), y)
                if params[i]['value'] != 'val1':
            ...
            for j in range(len(params)):
                for k in range(len(params)):
                    if params[i]['key'] != 'key1' and params[j]['key'] != 'key1' and params[k]['key'] != 'key1':
                        continue
                    query = (
                                        "select coalesce(%s,%s,%s) from " + self.query_bucket + 
                                        " USE KEYS ['plain'] where key1='" +
                                        self.scalars_test_data['not_null_1']['value'] + "'") % (params[i]['key'],
                                                                                                params[j]['key'],
                                                                                                params[k]['key'])

                    # expected_result is 'y' parameter in lambda - cannot be used directly since 
                    # lambdas lazy calculation
                    expected_result = self.null_to_none(params[i]['value'])
                    if params[i]['value'] != 'val1':
                        expected_result = self.null_to_none(params[j]['value'])
    ...
    def test_nvl_function_name(self):
        ...
        for func_name in self.nvl_function_names:
            query = ("select %s(key1, key2) from " + self.query_bucket + " USE KEYS ['plain'] where key1='" +
    ...
    def test_nvl2_function_name(self):
        ...
        for func_name in self.nvl2_function_names:
            query = ("select %s(key1,key1,key1) from " + self.query_bucket + " USE KEYS ['plain'] where key1='" +
                     self.scalars_test_data['not_null_1']['value'] + "'") % func_name
        ...
        for i in range(len(param1)):
            for j in range(len(param2)):
                for k in range(len(param3)):
                    ...
                    query = ("select nvl2(%s,%s,%s) from " + self.query_bucket + " USE KEYS ['plain'] where key1='" +
                             self.scalars_test_data['not_null_1']['value'] + "'") % \
                            (param1[i]['key'], param2[j]['key'], param3[k]['key'])

                    lambda1 = lambda x: self.assertTrue(x['q_res'][0]['status'] == 'success')
                    lambda2 = lambda x, y=self.null_to_none(param3[k]['value']): self.assertEqual(
                        self.normalize_result(x['q_res'][0]), y)
    ...
    def test_nvl2_different_nulls(self):
        ...
        for param in params:
            ...
            query = ("select nvl2(%s, " + self.scalars_test_data['not_null_1']['key'] + ", " +
                     self.scalars_test_data['not_null_2'][
                         'key'] + ") from " + self.query_bucket + " USE KEYS ['plain'] where key1='" +
    ...
    def test_coalesce_all_nulls(self):
        query = "select coalesce(" + self.scalars_test_data['null']['key'] + ", " + self.scalars_test_data['null'][
            'key'] + ", " + \
                self.scalars_test_data['null'][
                    'key'] + ") from " + self.query_bucket + " USE KEYS ['plain'] where key1='" + \
                self.scalars_test_data['not_null_1']['value'] + "'"
    ...
    def test_coalesce_null_string(self):
        ...
        params = [self.scalars_test_data['string_NULL'], self.scalars_test_data['string_NuLl'],
                  self.scalars_test_data['string_null']]
    ...
    def test_coalesce_empty_string(self):
        query = "select coalesce(" + self.scalars_test_data['null']['key'] + ", " + \
                self.scalars_test_data['empty_string']['key'] + ", " + \
                self.scalars_test_data['not_null_1'][
                    'key'] + ") from " + self.query_bucket + " USE KEYS ['plain'] where key1='" + \
                self.scalars_test_data['not_null_1']['value'] + "'"
    ...
    def test_nvl2_empty_string(self):
        query = "select nvl2(" + self.scalars_test_data['empty_string']['key'] + ", " + \
                self.scalars_test_data['not_null_1']['key'] + \
                ", " + self.scalars_test_data['not_null_2'][
                    'key'] + ") from " + self.query_bucket + " USE KEYS ['plain'] where key1='" + \
                self.scalars_test_data['not_null_1']['value'] + "'"
    ...
    def test_nvl_empty_string(self):
        query = "select nvl(" + self.scalars_test_data['empty_string']['key'] + ", " + \
                self.scalars_test_data['not_null_1']['key'] + \
                ") from " + self.query_bucket + " USE KEYS ['plain'] where key1='" + \
                self.scalars_test_data['not_null_1']['value'] + "'"
    ...
    def test_coalesce_multiple_nulls(self):
        ...
        query = "select coalesce(" + self.scalars_test_data['not_null_1']['key'] + ", " + \
                self.scalars_test_data['null']['key'] + ", " + \
                self.scalars_test_data['not_null_2'][
                    'key'] + ") from " + self.query_bucket + " USE KEYS ['plain'] where key1='" + \
                self.scalars_test_data['not_null_1']['value'] + "'"
    ...
    def load_data_for_scalar_tests(self):
        # query = "insert into " + self.query_bucket + " values ('plain',{'key1':'val1','key2':'val2','key3':null,...})"
        counter = 0
        query = "insert into " + self.query_bucket + " values('plain',{"
