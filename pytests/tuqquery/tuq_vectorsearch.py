...
class VectorSearchTests(QueryTests):
    ...
    def test_ann_knn(self):
        ...
        try:
            ...
            IndexVector().create_index(self.database,similarity=self.distance, is_xattr=self.use_xattr, is_base64=self.use_base64, network_byte_order=self.use_bigendian, description=self.description, dimension=self.dimension, use_bhive=self.use_bhive,custom_index_fields="vec VECTOR", vector_type=self.vector_type)
            explain = self.run_cbq_query(explain_query)
            query_plan = explain['results'][0]['plan']
            children = query_plan['~children'][0]['~children'][0]
            index_name = children['index']
    ...
    def test_use_index(self):
        ...
        try:
            ...
            IndexVector().create_index(self.database, index_order=self.index_order, similarity="L2", is_xattr=self.use_xattr, is_base64=self.use_base64, network_byte_order=self.use_bigendian, description=self.description, dimension=self.dimension, train=self.train, use_bhive=self.use_bhive, vector_type=self.vector_type)
            IndexVector().create_index(self.database, index_order=self.index_order, similarity="EUCLIDEAN", is_xattr=self.use_xattr, is_base64=self.use_base64, network_byte_order=self.use_bigendian, description=self.description, dimension=self.dimension, train=self.train, use_bhive=self.use_bhive, vector_type=self.vector_type)
            explain = self.run_cbq_query(explain_query)
            query_plan = explain['results'][0]['plan']
            children = query_plan['~children'][0]['~children'][0]
            index_name = children['index']
    ...
    def test_select_pushdown_index(self):
        ...
        try:
            ...
            IndexVector().create_index(self.database,similarity=self.distance, is_xattr=self.use_xattr, is_base64=self.use_base64, network_byte_order=self.use_bigendian, description=self.description, dimension=self.dimension, train=self.train, use_bhive=self.use_bhive,custom_index_fields="vec VECTOR",custom_name="non_pushdown",use_partition=self.use_partition,nprobes=self.nprobes, vector_type=self.vector_type)
            explain_plan = self.run_cbq_query(explain_query)
            # To know pushdown happens we need to see index_order and that the spans have the same high and low values, and Order is not an operator in the plan
            self.assertTrue('index_order' in str(explain_plan), f'We expect order to be pushed to the indexer, please check plan {explain_plan}')
            if self.use_bhive:
    ...
    def check_results_against_knn(self, expected_results, ann_query):
        ...
        ann_results = self.run_cbq_query(ann_query)
        recall, accuracy = UtilVector().compare_result(expected_results, ann_results['results'])
        self.log.info(f'Recall rate: {round(recall, 2)}% with acccuracy: {round(accuracy,2)}%')
        if recall < threshold:
            self.fail(f"Recall rate of {recall} is less than expected {threshold}")
    ...
    def test_aggregate_pushdown_negative(self):
        ...
        try:
            # Index is on (vec VECTOR)
            IndexVector().create_index(self.database,similarity=self.distance, is_xattr=self.use_xattr, is_base64=self.use_base64, network_byte_order=self.use_bigendian, description=self.description, dimension=self.dimension, train=self.train, use_bhive=self.use_bhive,custom_index_fields="vec VECTOR",nprobes=self.nprobes, vector_type=self.vector_type)
            ann_results = self.run_cbq_query(ann_query)
