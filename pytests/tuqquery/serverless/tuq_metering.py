...
class QueryMeteringTests(ServerlessBaseTestCase):
    ...
    def test_js_udf_n1ql(self):
        ...
        result = self.run_query(f"CREATE or REPLACE FUNCTION default:{self.bucket}._default.select_js(city) LANGUAGE JAVASCRIPT as 'function select_js(city) {{\
            var query = SELECT name FROM _default WHERE name = $city;\
            var acc = [];\
        ...
        before_kv_ru, before_kv_wu = meter.get_kv_rwu(database.id)
        result = self.run_query(f"EXECUTE FUNCTION default:{self.bucket}._default.select_js('Paris')")
        after_kv_ru, after_kv_wu = meter.get_kv_rwu(database.id)
        self.assertEqual(before_kv_ru, after_kv_ru) # no kv write
    ...
    def test_kv_inline_udf(self):
        ...
        before_kv_ru, before_kv_wu = meter.get_kv_rwu(database.id)
        result = self.run_query(f"CREATE or REPLACE FUNCTION select_func() {{(SELECT * FROM {self.collection})}}")
        after_kv_ru, after_kv_wu = meter.get_kv_rwu(database.id)
        self.assertEqual(before_kv_ru, after_kv_ru)
        self.assertEqual(before_kv_wu, after_kv_wu)
        self.assertTrue('billingUnits' not in result)
        ...
        expected_ru = self.doc_count * math.ceil( (self.doc_size + self.doc_key_size) / self.kv_ru)
        result = self.run_query(f'EXECUTE FUNCTION select_func()')
        assert_query, msg = meter.assert_query_billing_unit(result, expected_ru + sc_ru, unit='ru', service='kv')
        self.assertTrue(assert_query, msg)
    ...
    def test_no_rw_scope(self):
        ...
        self.run_query(f'CREATE COLLECTION {self.bucket}.s1.c1')
        self.sleep(2)
        self.run_query(insert_query)
        self.run_query(f'CREATE INDEX idx_name on {self.bucket}.s1.c1(name)')
    ...
    def test_eval_cu(self):
        ...
        self.create_users()
        self.run_query(f"GRANT query_select ON {self.collection} to {user_id}")
