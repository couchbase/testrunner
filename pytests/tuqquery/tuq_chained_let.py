...
class QueryChainedLetTests(QueryTests):
    ...
    def test_basic_chained_let(self):
        ...
        query_10 = 'select a, b, c from default d0 let usekeys=(select meta(d1).id, join_day from default d1), a=(select join_day from default d2), b=(select join_mo from default d3 use keys usekeys.id[*] where join_mo in a.join_yr[*]), c=(select join_yr from default d4 use keys usekeys.id[*] where join_mo in b.join_mo[*]) order by a, b, c limit 10'
        verify_10 = 'select (select join_day from default d1) as a, (select join_mo from default d2 where join_mo in (select join_day from default d3)) as b, (select join_yr from default d4 where join_mo in (select join_mo from default d5 where join_mo in (select join_day from default d6))) as c from default d7 order by a, b, c limit 10'
