...
class BaseGSIServerless(QueryBaseServerless):
    ...
    def wait_until_indexes_online(self, database, index_name, keyspace, indexer_node, timeout=20, replica_check=True):
        ...
        while time.time() - time_now < timeout*60:
            ...
            ##TODO add index_replica_online check after MB-53763
            # if index_online and index_replica_online:
            if index_online:
                if replica_check:
                    if index_replica_online:
                        break
                else:
                    break
            time.sleep(30)
            ##TODO add index_replica_online check after MB-53763
            # if not (index_online and index_replica_online):
        if not index_online:
            raise Exception(f"Index {index_name} on database {database} not online despite waiting for {timeout} mins")
        if replica_check and index_replica_online:
