...
class RebalanceInOutTests(RebalanceBaseTest):
    ...
    def incremental_rebalance_in_out_with_mutation_and_expiration(self):
        ...
        for i in reversed(range(self.num_servers)[self.num_servers / 2:]):
            tasks = self._async_load_all_buckets(self.master, self.gen_update, "update", 0, batch_size=500)
            tasks.extend(self._async_load_all_buckets(self.master, gen_expire, "update", 5, batch_size=500))
