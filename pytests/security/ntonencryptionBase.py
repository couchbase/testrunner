...
class ntonencryptionBase:
    ...
    PORTS_INDEXER = [9100,9101,9105]
    #9102,9103,9104
    PORTS_ANALYTICS = [9110,9111,9112,9113,9115,9116,9117,9118,9119,9121]
    #9120,9110,9112
    PORTS_QUERY = [18093]
    PORTS_FTS = [18094]
    ...
    def setup_nton_cluster(self,servers,ntonStatus='enable',clusterEncryptionLevel='control'):
        ...
        self.ntonencryption_cli(servers,ntonStatus)
        ...
        for node in servers:
            ...
            for service in node_services_list:
                ...
                for node_port in node_ports:
                    ...
                    log.info(" --- Port being Tested --{0} ---- and service is ---{1} ------".format(node_port,service))
    ...
    def validate_results(self, servers, results, clusterLevelEncryption):
        finalResult = True
        for node in servers:
            node_info = "{0}:{1}".format(node.ip, node.port)
            rest = RestConnection(node)
