    ...
    def sdk_get_raw(self, cluster, bucket, key, scope=None, collection=None):
        ...
        try:
            ...
            try:
                ...
                # Robust across couchbase SDK 3.x and 4.x. The SDK 3.x
                # DocumentNotFoundException stringifies as
                # "...RC=0x12D[LCB_ERR_DOCUMENT_NOT_FOUND (301)]..." --
                # it contains neither "DocumentNotFoundException" nor
                # "document_not_found", so a str()-only check re-raises a
                # legitimate "doc absent" as an error. Match the
                # exception TYPE name (identical on both SDKs) first.
                type_name = type(e).__name__
                lowered = str(e).lower()
                if ("DocumentNotFound" in type_name
                        or "document_not_found" in lowered
                        or "lcb_err_document_not_found" in lowered
                        or "key_enoent" in lowered):
                    return None
                raise
        finally:
            try:
