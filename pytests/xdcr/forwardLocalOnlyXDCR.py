...
class _SharedSDKClient:
    ...
    # ------------------------------------------------------------------
    # SDK 3.x subdoc compatibility (morpheus). The morpheus SDKClient
    # wrappers `lookup_in`/`mutate_in` are SDK-4-incompatible with how
    # this suite calls them: their signature is
    # `(key, scope=None, collection=None, *specs, **kwargs)`, so a
    # `specs=[...]` keyword lands in **kwargs and is forwarded to the
    # collection as `lookup_in(key, specs=[...])` -- but the SDK 3.x
    # `Collection.lookup_in(key, spec, ...)` requires `spec` as a
    # POSITIONAL list, raising "missing 1 required positional argument:
    # 'spec'". They also return nothing. Override here so callers get
    # the correct positional spec list AND the result back. These are
    # only used on single-threaded verification paths; parallel loaders
    # use their own dedicated connections (see `_sdk_parallel_map`).
    # ------------------------------------------------------------------
    def lookup_in(self, key, scope=None, collection=None, specs=None,
                  **kwargs):
        spec_list = list(specs) if specs else []
        if scope or collection:
            self._real.collection_connect(scope, collection)
            coll = self._real.collection
        else:
            coll = self._real.default_collection
        return coll.lookup_in(key, spec_list)

    def mutate_in(self, key, scope=None, collection=None, specs=None,
                  **kwargs):
        spec_list = list(specs) if specs else []
        if scope or collection:
            self._real.collection_connect(scope, collection)
            ...
            coll = self._real.default_collection
        return coll.mutate_in(key, spec_list)
...
class ForwardLocalOnlyXDCRBase(XDCRNewBaseTest):
    ...
    def _new_sdk_connection(self, cluster, bucket_name,
                            scope=None, collection=None):
        """Open a DEDICATED SDK connection and resolve the target
        collection. Parallel loaders need one connection per worker:
        the SDK 3.x connection on morpheus is not thread-safe, so a
        single shared connection raises "Couldn't lock ... using the
        Connection object from multiple threads" under a thread pool.
        Returns (client, collection); caller owns closing the client."""
        master = cluster.get_master_node()
        client = SDKClient(
            bucket=bucket_name,
            hosts=[master.ip],
    ...
    def _sdk_parallel_map(self, cluster, bucket_name, items, work,
                          scope=None, collection=None, thread_count=None):
        """Apply `work(coll, item)` to every item, giving each worker
        thread its OWN SDK connection drawn from a bounded pool (SDK 3.x
        connections are not thread-safe; a shared one fails under a
        thread pool). Connections are opened up front and closed when
        the batch finishes. Returns a list, order-preserving, of None
        on success or an error string on failure -- callers tally
        successes/failures themselves."""
        items = list(items)
        if not items:
            return []
        thread_count = thread_count or self._input.param(
            "bulk_threads", self._BULK_THREADS)
        workers = (min(thread_count, len(items))
                   if thread_count and thread_count > 1 else 1)
        conns = [self._new_sdk_connection(cluster, bucket_name,
                                          scope, collection)
        ...
        def _run(item):
            ...
            try:
                ...
                pool_q.put((client, coll))

        try:
            if workers > 1 and len(items) > 1:
                with ThreadPoolExecutor(max_workers=workers) as pool:
                    return list(pool.map(_run, items))
            return [_run(it) for it in items]
        finally:
            for client, _coll in conns:
                try:
                    client.close()
                except Exception as e:
                    self.log.warning(
                        "{0} parallel SDK connection close failed "
                        "(non-fatal): {1}".format(self._tag(), e))
    ...
    def _bulk_upsert(self, cluster, bucket, key_value_pairs,
        ...
        Each parallel batch uses its OWN SDK connection: the SDK 3.x
        connection on morpheus is not thread-safe, so a single shared
        connection fails under a thread pool with LCB lock errors."""
        ...
        errs = self._sdk_parallel_map(
            cluster, bucket.name, batches,
            lambda coll, batch: coll.upsert_multi(batch),
            scope=scope, collection=collection,
            thread_count=thread_count)
        sent = sum(len(b) for b, e in zip(batches, errs) if e is None)
    ...
    def _sdk_subdoc_xattr_mutate(self, cluster, bucket, key_prefix, count,
        ...
        thread pool. Each worker uses its OWN SDK connection -- the
        SDK 3.x connection on morpheus is not thread-safe -- and calls
        the raw collection's `mutate_in(key, spec_list)` directly (the
        morpheus SDKClient.mutate_in wrapper mis-routes `specs=` and
        returns nothing; see `_SharedSDKClient.mutate_in`)."""
        self.log.info(
        ...
        def _mutate(coll, i):
            ...
            coll.mutate_in(key, [
                SD.upsert(
                    "flo_meta.tag",
                    "local-{0}-{1}".format(cluster.get_name(), i),
        ...
        errs = self._sdk_parallel_map(
            cluster, bucket.name, range(start, start + count), _mutate)
        errors = [e for e in errs if e]
        ok = len(errs) - len(errors)
        if errors:
            self.log.warning(
