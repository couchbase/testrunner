function OnUpdate(doc, meta) {
    try {
        ...
        log("Running Disjuncts Query");
        var matchQueryForCompound = couchbase.SearchQuery.match("location")
            .field("reviews.content");
        var booleanQueryForCompound = couchbase.SearchQuery.booleanField(false)
            .field("free_breakfast");
        var disjunctsQuery = couchbase.SearchQuery.disjuncts(matchQueryForCompound, booleanQueryForCompound);
        writeQueryResult("disjunctsQuery", runQuery(disjunctsQuery, { size: 801 }), meta);
    } catch (e) {
