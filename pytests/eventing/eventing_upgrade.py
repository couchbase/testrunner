...
class EventingUpgrade(NewUpgradeBaseTest, EventingBaseTest):
    def setUp(self):
        """Initialize upgrade test: REST connections, bucket names, handler tracking."""
    ...
    def tearDown(self):
        """Clean up analytics link, then delegate to parent tearDown."""
    ...
    def test_offline_upgrade_with_eventing(self):
        ...
        # Post-upgrade features: collections, analytics, new handlers
    ...
    def test_online_upgrade_with_swap_rebalance_with_eventing(self):
        ...
        self._set_memory_quotas()
        # Post-upgrade features: collections, travel-sample, analytics, new handlers
        self._create_event_scope_collections()
    ...
    def _enable_tls_if_configured(self):
        ...
        if self.enable_n2n_encryption_and_tls:
            ...
            self.sleep(120, "Sleeping 2 min after enabling strict TLS encryption")
    ...
    def _create_event_scope_collections(self):
        ...
        for collection_name in (self.on_deploy_collection_name, self.curl_collection_name,
                                self.analytics_collection_name):
    ...
    def _deploy_post_upgrade_handlers(self, include_analytics=True):
        """Deploy post-upgrade (collection-scoped) handlers; pass
        include_analytics=False to skip the travel-sample-dependent ones."""
        self.add_built_in_server_user()
        self.restServer = self.get_nodes_from_services_map(service_type="eventing")
        self.rest = RestConnection(self.restServer)
        ...
        # Optional: analytics handler (requires travel-sample)
        if include_analytics:
            ...
            post_upgrade_handlers.extend(["analytics"])
        # Deploy all handlers
    ...
    def _run_full_mutation_cycle(self, include_analytics=True):
        ...
        # Verify analytics handler if deployed
        if include_analytics:
    ...
    def _run_pause_resume_cycle(self, include_analytics=True):
    ...
    def _form_cluster_low_quota(self, servers, services):
        """Form the initial cluster one service at a time, starting from a
        single node, so memory quotas (set right after that first node is up)
        don't have to account for every service coming up simultaneously."""
    ...
    def _refresh_after_online_upgrade(self):
        """Re-point cached handles (REST, server, buckets) at the
