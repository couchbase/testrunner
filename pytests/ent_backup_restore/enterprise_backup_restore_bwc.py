...
class EnterpriseBackupRestoreBWCTest(EnterpriseBackupRestoreBase, NewUpgradeBaseTest):
    def setUp(self):
        ...
        self.backupset.bkrs_client_upgrade_version = \
                              self.input.param("bkrs_client_upgrade_version", None)
        ...
        if self.backupset.bkrs_client_version is None and \
                                              not self.backupset.bkrs_client_upgrade:
            self.fail("\nTest needs to get cbm version to run test")
        ...
        if self.backupset.bkrs_client_upgrade:
            if self.backupset.current_bkrs_client_version[:5] > self.bk_cluster_version[:5]:
                self.log.info("\n**** Need to downgrade CBM version to {0}"\
                                                    .format(self.bk_cluster_version))
                self._install([self.backupset.backup_host], self.bk_cluster_version)
                self.backupset.current_bkrs_client_version = \
                                            self._get_current_bkrs_client_version()
        elif self.backupset.bkrs_client_version[:5] != \
                                        self.backupset.current_bkrs_client_version[:5]:
    ...
    def test_backup_restore_sanity_bwc(self):
        ...
        for i in range(1, self.backupset.number_of_backups + 1):
            ...
            self.sleep(10)
            if self.backupset.bkrs_client_upgrade:
                if self.backupset.bkrs_client_upgrade_version is None:
                    self.fail("\nNeed bkrs client upgrade version to run bkrs client upgrade")
                self.backupset.current_bkrs_client_version = \
                    self._get_current_bkrs_client_version()
                if i == 3 and self.backupset.number_of_backups >= 5 and \
                    ...
                    self.log.info("\nNeed to upgrade CBM version to {0} to run bkrs bwc upgrade"\
                                      .format(self.backupset.bkrs_client_upgrade_version))
