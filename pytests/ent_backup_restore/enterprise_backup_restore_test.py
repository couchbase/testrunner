...
class EnterpriseBackupRestoreTest(EnterpriseBackupRestoreBase, NewUpgradeBaseTest):
    ...
    def test_continuous_backup_basic_test(self):
        '''
            ...
            5. Wait for {interval_time} seconds and record the timestamp
            6. Upsert the documents and record the timestamp again
            7. Wait for {interval_time} seconds
    ...
    def test_continuous_backup_with_ttl(self):
        self.log.info("Testing continuous backup with TTL")

        self.log.info("1. Testing continuous backup with TTL set for documents")
        bucket = "ttl-bucket"
        interval_time = self.input.param("interval_time", 2)
        ...
        doc_value = {"num": 1}
        for i in range(self.num_items):
            result = sdk_client.insert("doc_" + str(i), doc_value, ttl=180, collection=collection_name)
            self.log.info("{}: The output is - {}".format(i, result))
        ...
        self.sleep(int(interval_time) * 60 + 10, "Waiting for continuous backup interval to lapse and documents to expire")
        ...
        self.log.info("2. Testing continuous backup with TTL set for collection")

        collection_name = "col2"
        collections_cli.create_collection(bucket=bucket, scope="_default", collection=collection_name, max_ttl=180)
        ...
        second_upsert_time = self.get_timestamp(self.master)[0]
        self.log.info("The upsert time is - {}".format(second_upsert_time))

        self.sleep(int(interval_time) * 60 + 10, "Waiting for documents to expire")

        output, error = self.backup_restore()
        self.log.info("The output of the traditional restore command is - {}".format(output))
        self.log.info("The error of the traditional restore command is - {}".format(error))

        output, error = self.restore_continuous_backup(second_upsert_time)
        self.log.info("The output of the traditional restore command is - {}".format(output))
        self.log.info("The error of the traditional restore command is - {}".format(error))

        self.log.info("3. Testing continuous backup with TTL set for bucket")

        output, error = self.create_bucket_for_continuous_backup(bucket, interval_time, max_ttl=180)
        self.log.info("The create bucket output is - {}".format(output))
        self.log.info("The create bucket error is - {}".format(error))
        ...
        third_upsert_time = self.get_timestamp(self.master)[0]
        self.log.info("The upsert time is - {}".format(third_upsert_time))
        ...
        output, error = self.restore_continuous_backup(third_upsert_time)
        self.log.info("The output of the traditional restore command is - {}".format(output))
        self.log.info("The error of the traditional restore command is - {}".format(error))

    def test_continuous_backup_with_rebalance(self):
        '''
            Need at least 2 nodes for this test
        '''
        self.log.info("Testing continuous backup with rebalance")

        bucket = "ttl-bucket"
        interval_time = self.input.param("interval_time", 2)
        replica_number = self.input.param("bucket_replica_number", 0)

        output, error = self.create_bucket_for_continuous_backup(bucket, interval_time, replica_number)
        self.log.info("The create bucket output is - {}".format(output))
        self.log.info("The create bucket error is - {}".format(error))
        ...
        # Take a traditional backup and continuous backup
        output, error = self.backup_create()
        self.log.info("The output of backup cluster command is -", output)
        self.log.info("The error of backup cluster command is -", error)
        ...
        rest_conn.add_node(self.master.rest_username, self.master.rest_password,
                           self.input.clusters[0][-1].ip)
        rebalance = self.cluster.async_rebalance(self.cluster_to_restore, [], [])
        rebalance.result()
    ...
    def test_continuous_backup_with_failover(self):
        '''
         Test should be run with 4 nodes - 3 nodes for cluster and 1 node for taking backup
        '''
        self.log.info("Testing continuous backup with failover")
        ...
        print(f"Upserting {self.num_items} documents in {bucket}->_default->_default...")
        ...
        nodes_all = rest.node_statuses()
        for node in nodes_all:
            if node.ip == self.servers[2].ip:   # Specifically this line considers there are 3 nodes in cluster
                rest.fail_over(otpNode=node.id, graceful=self.graceful)
                self.sleep(30)
                try:
                    rest.set_recovery_type(otpNode=node.id, recoveryType=self.recoveryType)
