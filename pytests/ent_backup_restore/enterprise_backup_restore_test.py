...
class EnterpriseBackupRestoreTest(EnterpriseBackupRestoreBase, NewUpgradeBaseTest, EventingBaseTest):
    ...
    def test_backup_resume(self):
        ...
        if num_vbuckets:
            ...
            command = "curl -X POST -u {}:{}".format(self.master.rest_username, self.master.rest_password)
            command += " {}:8091/diag/eval -d 'ns_config:set(couchbase_num_vbuckets_default, {}).'".format(self.master.ip, num_vbuckets)
            output, _ = remote_client.execute_command(command)
            if 'ok' not in output[0]:
                self.fail("failed to reduce the number of vBuckets {}".format(num_vbuckets))
        gen = BlobGenerator("ent-backup", "ent-backup-", self.value_size, end=self.num_items)
        self._load_all_buckets(self.master, gen, "create", 0)
        self.backup_create()
    ...
    def test_objstore_negative_args(self):
        remote_client = RemoteMachineShellConnection(self.backupset.backup_host)
        command = "{}/cbbackupmgr".format(self.cli_command_location)
        ...
        for sub_command in ['backup -a archive -r repo -c localhost -u admin -p password',
            ...
            for argument in ['--obj-access-key-id asdf',
                ...
                output, error = remote_client.execute_command("{} {} {}".format(command, sub_command, argument))
                remote_client.log_command_output(output, error)
            ...
            for argument in ['--obj-access-key-id',
                ...
                output, error = remote_client.execute_command(
                    "{} {} --obj-staging-dir staging {}"
                    .format(command, sub_command.replace('archive', self.objstore_provider.schema_prefix() + 'archive'), argument)
                )
                remote_client.log_command_output(output, error)
                self.assertNotEqual(len(output), 0)
                self.assertIn("Expected argument for option: {}".format(argument), output[0],
            ...
            for argument in ['--s3-log-level']:
                output, error = remote_client.execute_command(
                    "{} {} --obj-staging-dir staging {}".format(command, sub_command.replace('archive', 's3://archive'), argument)
                )
                remote_client.log_command_output(output, error)
                self.assertNotEqual(len(output), 0)
                self.assertIn("Expected argument for option: {}".format(argument), output[0],
            ...
            output, error = remote_client.execute_command(
                "{} {}".format(command, sub_command.replace('archive', self.objstore_provider.schema_prefix() + 'archive'))
            )
            remote_client.log_command_output(output, error)
            self.assertNotEqual(len(output), 0)
    ...
    def test_backup_cluster_restore_negative_args(self):
        ...
        error_message = "archive directory '{0}abc' does not exist".format(self.root_path)
        if self.cb_version[:5] <= "6.5.2":
            error_message = "Archive directory `abc` doesn't exist"
        self.assertTrue(self._check_output("{0}".format(error_message), output), "Expected error message not thrown")
        ...
        error_message = "Error {0} cluster: lookup abc ".format(part_message)
        if self.cb_version[:5] <= "6.5.2":
            error_message = "server misbehaving"
        self.assertTrue(self._check_output("{0}".format(error_message), output),
                        "Expected error message not thrown")
    ...
    def test_backup_list_negative_args(self):
        ...
        remote_client.log_command_output(output, error)
        remote_client.disconnect()
        error_message = "Error getting archive information: archive directory '{0}abc' does not exist".format(self.root_path)
        if self.cb_version[:5] <= "6.5.2":
            error_message = "Archive directory `abc` doesn't exist"
        self.assertTrue("{0}".format(error_message) in output[-1],
                        "Expected error message not thrown")
    ...
    def test_backup_compact_negative_args(self):
        ...
        output, error = remote_client.execute_command(command)
        remote_client.log_command_output(output, error)
        error_message1 = "Error opening archive at {0}abc due to `Not an archive directory`".format(self.root_path)
        error_message2 = "Compacting incr backup `backup` of backup `abc` failed:"
        if self.cb_version[:5] <= "6.5.2":
            error_message1 = "Error opening archive at abc due to `Not an archive directory`"
            error_message2 = "Compacting incr backup `abc` of backup `backup` failed:"
        self.assertTrue("{0}".format(error_message1) in output[-1],
                        "Expected error message not thrown")
        ...
        remote_client.log_command_output(output, error)
        remote_client.disconnect()
        self.assertTrue("{0}".format(error_message2) in output[-1],
                        "Expected error message not thrown")
    ...
    def test_backup_merge_negative_args(self):
        ...
        output, error = remote_client.execute_command(command)
        remote_client.log_command_output(output, error)
        error_message = "Error merging data: archive directory '{0}abc' does not exist".format(self.root_path)
        if self.cb_version[:5] <= "6.5.2":
            error_message = "Archive directory `abc` doesn't exist"
        self.assertTrue("{0}".format(error_message) in output[-1],
                        "Expected error message not thrown")
    ...
    def test_backup_remove_negative_args(self):
        ...
        command = "{0}/cbbackupmgr {1}".format(self.cli_command_location, cmd)
        output, error = remote_client.execute_command(command)
        error_message = "Removing backup repository failed: archive directory '{0}abc' does not exist".format(self.root_path)
        if self.cb_version[:5] <= "6.5.2":
            error_message = "Archive directory `abc` doesn't exist"
        remote_client.log_command_output(output, error)
        self.assertTrue("{0}".format(error_message) in output[-1],
    ...
    def test_backup_logs_for_keywords(self):
        ...
        for keyword, at_start, lines_before, lines_after in bad_keywords:
            ...
            if found:
                self.fail("Found bad keyword(s) '{}' in backup logs:\n".format(keyword) + "\n".join(output))
    ...
    def test_backup_remove_take_backup_range(self):
        ...
        command += "{}/{}".format(self.backupset.directory, self.backupset.name)
    ...
    def test_info_backup_merge_remove(self, cluster, no_of_backups):
        ...
        self.backup_merge()
        del self.backups[self.backupset.start - 1:self.backupset.end - 1]
    ...
    def test_info_after_backup_merge_remove(self):
        ...
        try:
            ...
            def output_logs(flag_depth, use_json, all_flag):
                ...
                self.log.info("---")
                self.log.info("Testing Flags: {0} {1} {2}".format(flags, use_json, all_flag))
                self.log.info("---")
