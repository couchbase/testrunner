...
parser = argparse.ArgumentParser(
    description='This script changes Indexer settings, create appropriate settings.json to upload')
...
t = os.system("curl {0} -u Administrator:password -d  @settings.json".format(url))
print("curl {0} -u Administrator:password -d  @settings.json".format(url))
print t

# For individual changes on memory quota and indexer threads
...
#print m
#r=os.system("curl -X POST -u Administrator:password http://10.6.2.167:8091/settings/indexes -d indexerThreads=8")
#print r
