...
#	p <- p + geom_line(aes(timestamp, ops, color=build))
	...
	print(testName)
	values <- c(paste('test:', testName), paste('items:', temp_data_frame$total_items[1]), paste('value_size:',comma(temp_data_frame$min_value_size[1])), paste('total_gets:', comma(temp_data_frame$total_gets[1])), paste('total_sets:', comma(temp_data_frame$total_sets[1])), paste('total_creates:', comma(temp_data_frame$total_creates[1])), paste('run_time:', temp_data_frame$run_time[1]) )
	print(values)
	print(p)
