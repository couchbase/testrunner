...
b <- b[b$test==tname,]
p <- ggplot(b,aes(rowid,mem_used,fill=unique_col)) + labs(x="",y="")
...
print(p)
p <- ggplot(b,aes(rowid,flusher_todo,fill=unique_col)) + labs(x="",y="")
...
xx <- fromJSON(file=paste("http://ec2-50-16-117-7.compute-1.amazonaws.com:5984/pythonperformance","/_design/rviews/_view/procstats", sep=''))$rows
bb <- plyr::ldply(xx, unlist)
