...
for(arg in args) {
	...
	for(a_build in levels(builds)) {
		...
		p <- ggplot(graphed,aes(rowid, drainRate, fill=build)) + labs(x="samples", y="ms")
	...
	for(a_build in levels(builds)) {
		...
		max_time <- max(filtered$test_time)
		graphed <- bb[bb$build == a_build & bb$test_time == max_time,]
		p <- ggplot(graphed,aes(rowid, avgLatency, fill=build)) + labs(x="samples", y="ms")
	...
	for(a_build in levels(builds)) {
		...
		max_time <- max(filtered$test_time)
		graphed <- bb[bb$build == a_build & bb$test_time == max_time,]
		p <- ggplot(graphed,aes(rowid, opsPerSecond, fill=build)) + labs(x="samples", y="OPS")
