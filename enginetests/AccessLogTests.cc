...
class AccessLogTest : public BaseTest {
		...
		void createView(){
				...
				char view[] = "curl -X PUT -H 'Content-Type: application/json'  "
				              "http://Administrator:password@localhost:8092/default/_design/getmeta  "
							  "-d @getmeta.ddoc";
				exec(view);
		} 
		...
		char getRevId(){
				...
				char getview[] = "curl http://Administrator:password@localhost:8092/default"
...
TEST_F(AccessLogTest, AccessLogTest_LogExists) {
		...
		for(uint64_t  i = 1;i<100000 ;i=i+100) {
				...
				for(int j = 0; j<100;j++)callget(&instance, myString.c_str(), myString.size());   
		}
		...
		if ((file0 = fopen(log0.c_str(), "r"))&&(file1 = fopen(log1.c_str(), "r"))&&
						(file2 = fopen(log2.c_str(), "r"))&&(file3 = fopen(log3.c_str(), "r"))) {
				fclose(file0); 
				fclose(file1); 
				fclose(file2); 
				fclose(file3); 
...
TEST_F(AccessLogTest, LoadFromOldAccessLogTest) {
		...
		if (File1!=NULL)
		{
                ...
                time_t log0oldacctime = f0info.st_atime;
                std::cout<<"first acccess time: "<<log0oldacctime<<"\n"; 
		loadfromAccessLogs();
                sleep(20);
                stat(log0.c_str(), &f0info); 
                std::cout<<"second acccess time: "<<f0info.st_atime<<"\n"; 
                EXPECT_GT(f0info.st_atime,log0oldacctime); 
		getwarmupcount();
		int wm_val_cn_i = atoi(wm_val_cn);
		printf("wamup value count: %d",wm_val_cn_i); 
