...
protected:
	...
	void deleteBucket(std::string bucketname)
	{
		...
		std::string del = "/opt/couchbase/bin/couchbase-cli bucket-delete -c 127.0.0.1:8091 --bucket=" + bucketname
	...
	void createBucketwithPriority(bool highprio, std::string bucketname)
	{
		if (!highprio)
		{
			std::string create = "/opt/couchbase/bin/couchbase-cli bucket-create -c 127.0.0.1:8091 --bucket=" + bucketname
			    + " --bucket-type=couchbase --bucket-ramsize=200 --bucket-replica=1"
