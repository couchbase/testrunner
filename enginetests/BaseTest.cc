...
protected:
	...
	virtual void SetUp()
	{
		...
		memset(&this->create_options, 0, sizeof(this->create_options));
		if (testargc > 0)
		{
			this->create_options.v.v0.host = testargv[1];
		}
		if (testargc > 1)
		{
			this->create_options.v.v0.user = testargv[2];
			this->create_options.v.v0.bucket = testargv[2];
		}
		if (testargc > 2)
		{
			this->create_options.v.v0.passwd = testargv[3];
		}
		err = lcb_create(&this->instance, &this->create_options);
		assert(err == LCB_SUCCESS);
		(void) lcb_set_error_callback(this->instance, error_callback);
		err = lcb_connect(this->instance);
		assert(err == LCB_SUCCESS);
		fprintf(stderr, "\nconnect succeeded\n");
		(void) lcb_set_get_callback(this->instance, get_callback);
		(void) lcb_set_store_callback(this->instance, store_callback);
		(void) lcb_set_stat_callback(this->instance, stats_callback);
	...
	void sendHello()
	{
		...
		{
			err = lcb_hello(this->instance, NULL);
			assert(err == LCB_SUCCESS);
		}
	...
	uint64_t insertItems(uint64_t numItems)
	{
		...
		for (uint64_t i = 0; i < numItems; i++)
		{
			...
			lcb_error_t err = lcb_store(this->instance, NULL, 1, &commands[0]);
			if (err == LCB_SUCCESS)
				numinserted++;
	...
	void verifyReplication(uint64_t numItems)
	{
		...
		for (uint64_t i = 0; i < numItems; i++)
		{
			...
			{ get };
			lcb_get_replica(this->instance, NULL, 1, commands);
			lcb_wait(this->instance);
		}
	...
	void insert_items_instances(uint64_t numItems, std::vector<lcb_t*> instanceVec)
	{
		...
		for (uint64_t i = 0; i < numItems; i++)
		{
			...
			{
				lcb_store(**itr, NULL, 1, &commands[0]);
				lcb_wait(**itr);
			}
		}
	}

	StatsVector getgenericstats(std::string const & statsreq, lcb_t& instance)
	{
	...
	void DatatypeTester(const lcb_store_cmd_t *commands)
	{
		...
		lcb_wait(this->instance);
		callget(&this->instance, commands->v.v0.key, commands->v.v0.nkey);
		lcb_wait(this->instance);
		fprintf(stderr, "\nInside DatatypeTester\n");
		this->compareDocs(commands);
	}
protected:
	lcb_uint32_t tmo;
	const lcb_store_cmd_t *commands[1];
