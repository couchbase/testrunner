...
def usage(err=None):
    if err:
        ...
        r = 0
    print """\
persistence_data_validation.py
 -h --help
 -v --verbose
...
if __name__ == "__main__":
    ...
    print "Evicting all the items from memory"
    for vbucket in range(config.vbuckets):
        evict_items(config.servers[0], vbucket, config.items)

    print "Loading all the items from the disk and validating them"
    passed = True
    valid_items = 0
    total_valid_items = 0
    for vbucket in range(config.vbuckets):
    ...
    if passed == True:
        print "Persistence data validation after evicting items: Passed"
    if passed == False:
        config.return_code = 1
        print "Persistence data validation after evicting items: Failed"

    print "Kill -9 one of the nodes, and start it again"
    server_killed = config.servers[len(config.servers) -1]
    kill_server(server_killed)
    start_server(server_killed)
    wait_on_warmup(server_killed)

    print "Validating all the items after restarting the server"
    passed = True
    valid_items = 0
    total_valid_items = 0
    for vbucket in range(config.vbuckets):
    ...
    print "Valid keys: %d / %d" % (total_valid_items, config.items * config.vbuckets)
    if passed == True:
        print "Persistence data validation after a server crash: Passed"
    if passed == False:
        config.return_code = 1
        print "Persistence data validation after a server crash: Failed"
