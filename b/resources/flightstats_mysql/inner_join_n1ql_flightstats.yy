...
ontime2carrier:
	ontime_table INNER JOIN carrier_table ON  KEYS [ previous_table.`carrier`];

first2carrier:
	INNER JOIN carrier_table ON KEYS [a1.`carrier`];

ontime2airport:
	ontime_table INNER JOIN airport_table ON  KEYS [ previous_table.origin_destination ];

first2airport:
	INNER JOIN airport_table ON  KEYS [ a1.origin_destination ];
	
origin_destination:
	`origin` | `destination` ;

ontime2aircraft:
	ontime_table INNER JOIN aircraft_table ON KEYS [ previous_table .`tail_num`];

first2aircraft:
	INNER JOIN aircraft_table ON KEYS [ a1.`tail_num`];

airport2state:
	airport_table INNER JOIN state_table ON  KEYS [ previous_table.`state`];

airport2zipcode:
	airport_table INNER JOIN zipcode_table ON  KEYS [ previous_table.`state` ];

airport2remark:
	airport_table INNER JOIN remark_table USING KEYS [ previous_table.`site_number` ] ;

aircraft2engine:
	aircraft_table INNER JOIN engine_table USING KEYS [ previous_table.`aircraft_engine_code` ] ;
