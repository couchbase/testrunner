...
ontime2airport:
	ontime_table  JOIN airport_table ON ( previous_table.origin_destination = current_table .`code` ) ;

first2airport:
	LEFT JOIN airport_table ON ( a1 .origin_destination = current_table .`code` );
	
origin_destination:
	`origin` | `destination` ;

ontime2aircraft:
	ontime_table LEFT OUTER JOIN aircraft_table ON ( previous_table.`tail_num` = current_table.`tail_num` ) ;

first2aircraft:
	INNER JOIN aircraft_table ON ( a1.`tail_num` = current_table.`tail_num` );

airport2state:
	airport_table  JOIN state_table ON ( previous_table.`state` = current_table.`state_code` );

airport2zipcode:
	airport_table LEFT OUTER JOIN zipcode_table ON ( previous_table.`state` = current_table.`state_code` );

airport2remark:
	airport_table INNER JOIN remark_table USING ( `site_number` ) ;

aircraft2engine:
	aircraft_table LEFT JOIN engine_table USING ( `aircraft_engine_code` ) ;
