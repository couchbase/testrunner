...
operations:
	(select) set_operation (select) | (select) set_operation (select) set_operation (select) | (select) set_operation (select) set_operation (select) set_operation (select);

set_operation:
	UNION | UNION ALL  ;

select:
	SELECT * FROM BUCKET_NAME WHERE complex_condition ORDER BY field_list nulls_first_last |
