...
class MemcachedError(Exception, metaclass=MemcachedErrorMetaclass):
    ...
    def __init__(self, status, msg):
        ...
        print("type of supermsg and status and msg :", type(supermsg),type(status),type(msg))
        # supermsg = bytes(supermsg, encoding='utf-8')
        if msg:
...
class MemcachedClient(object):
    ...
    def __init__(self, host='127.0.0.1', port=11211, family=socket.AF_UNSPEC,timeout = None):
        self.host = host
        self.port = port
