               ...
               time.sleep(POLL_INTERVAL)
                ...
                if haveTestToLaunch:
                    ...
                    if response.status == 499 or \
                        ...
                        if options.serverType.lower() == 'docker':
                            base_url_str = base_url_str + '-docker'
                        elif options.test:
                            base_url_str = base_url_str + '-test'
                        elif testsToLaunch[i]['framework'] == "jython":
                            base_url_str = base_url_str + '-jython'
                        elif testsToLaunch[i]['framework'] == "TAF":
                            base_url_str = base_url_str + '-TAF'
                        elif options.jenkins is not None:
                            base_url_str = base_url_str + '-' + options.jenkins
