...
then
  ...
  then
    ...
    export install_params=`$python_exe -c 'import sys;import jenkins; import os;import json; j = jenkins.Jenkins(os.environ["HUDSON_URL"]); paramDef=filter(None, j.get_job_info(os.environ["JOB_NAME"])["actions"])[0]["parameterDefinitions"]; [sys.stdout.write(p["defaultParameterValue"]["value"]) for p paramDef if p["name"] == "install_params"]'`
    export run_install=`$python_exe -c 'import sys;import jenkins; import os;import json; j = jenkins.Jenkins(os.environ["HUDSON_URL"]); paramDef=filter(None, j.get_job_info(os.environ["JOB_NAME"])["actions"])[0]["parameterDefinitions"]; [sys.stdout.write(p["defaultParameterValue"]["value"]) for p in paramDef if p["name"] == "run_install"]'`
