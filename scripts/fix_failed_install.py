...
def setpoolstate(poolId, ip, fromstate, tostate):
	query = "update `QE-server-pool` set state='%s' where ('%s' in poolId or poolId='%s') and state like '%s%%' and ipaddr='%s';" % (str(tostate),
	str(poolId), str(poolId), str(fromstate), str(ip))
...
def setipstate(ip, fromstate, tostate):
	query = "update `QE-server-pool` set state='%s' where state like '%s%%' and ipaddr='%s';" % (str(fromstate), str(tostate), str(ip))
	print('Running: %s' % query)
	row_iter = cb.n1ql_query(N1QLQuery(query))
	for row in row_iter:
		print(row)
...
def clean_failedinstsall_vms(poolId):
  ...
  query = 'select * from `QE-server-pool` where ("%s" in poolId or poolId="%s") and state like "failedInstall%%";' % (str(poolId),str(poolId))
  print('Running: %s' % query)
  row_iter = cb.n1ql_query(N1QLQuery(query))
  fixed_ips = []
  index=1
  for row in row_iter:
	try:
		...
		if out_2 == '':
			...
			setpoolstate(poolId, server, "failedInstall", "available")
...
parser = OptionParser()
parser.add_option("-a", "--action", dest="action", default="clean_failedinstsall_vms", help="Enter action")
